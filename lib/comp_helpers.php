...
{
    $db = getDB();
    $stmt = $db->prepare("UPDATE Arcade_Competitions set current_participants = (SELECT IFNULL(COUNT(1),0) FROM Arcade_UserComps WHERE competition_id = :cid), 
...
{
    $db = getDB();
    $stmt = $db->prepare("INSERT INTO Arcade_UserComps (user_id, competition_id) VALUES (:uid, :cid)");
    try {
...
function join_competition($comp_id, $user_id, $cost)
{
    ...
    if ($comp_id > 0) {
        if ($balance >= $cost) {
            $db = getDB();
            $stmt = $db->prepare("SELECT title, join_cost from Arcade_Competitions where id = :id");
            try {
...
function get_top_scores_for_comp($comp_id, $limit = 10)
{
    ...
    /*$stmt = $db->prepare(
        "SELECT score, s.created, username, u.id as user_id FROM Arcade_Scores s 
    JOIN Arcade_UserComps uc on uc.user_id = s.user_id 
    JOIN Arcade_Competitions c on c.id = uc.competition_id
    ...
    $stmt = $db->prepare("SELECT * FROM (SELECT s.user_id, s.score,s.created, a.id as account_id, DENSE_RANK() OVER (PARTITION BY s.user_id ORDER BY s.score desc) as `rank` FROM Arcade_Scores s
    JOIN Arcade_UserComps uc on uc.user_id = s.user_id
    JOIN Arcade_Competitions c on uc.competition_id = c.id
    JOIN Arcade_Accounts a on a.user_id = s.user_id
...
function calc_winners()
{
    ...
    from Arcade_Competitions c JOIN Arcade_Payout_Options po on c.payout_option = po.id 
    ...
    if (count($calced_comps) > 0) {
        $query = "UPDATE Arcade_Competitions set did_calc = 1, did_payout = 1 WHERE id in ";
    ...
    //close invalid comps
    $stmt = $db->prepare("UPDATE Arcade_Competitions set did_calc = 1 WHERE expires <= CURRENT_TIMESTAMP() AND current_participants < min_participants AND did_calc = 0");
    try {
        $stmt->execute();
        $rows = $stmt->rowCount();
