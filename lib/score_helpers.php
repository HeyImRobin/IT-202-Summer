...
function get_top_10($duration = "day")
{
    ...
    $db = getDB();
    $query = "SELECT user_id,username, Arcade_Scores.created from Arcade_Scores join Users on Arcade_Scores.user_id = Users.id";
    if ($d === "day") {

        $query .= " WHERE Arcade_Scores.created >= addtime(CURDATE(), '00:00:00') AND Arcade_Scores.created <= addtime(CURDATE(), '23:59:59')";
    } else if ($d === "week") {
        //https://stackoverflow.com/a/42540446
        // Change it to start on Sunday to Saturday 
        $query .= " WHERE 
        Arcade_Scores.created >= addtime(date_add(curdate(), interval  -WEEKDAY(curdate()) day), '00:00:00')
        AND
        Arcade_Scores.created <= addtime(date_add(date_add(curdate(), interval  -WEEKDAY(curdate())-1 day), interval 7 day), '23:59:59')";
    } else if ($d === "month") {
        //https://www.mysqltutorial.org/mysql-last_day/
        $query .= " WHERE 
        Arcade_Scores.created >=  addtime(dATE_SUB(curdate(),INTERVAL DAYOFMONTH(curdate())-1 DAY), '00:00:00')
        AND
        Arcade_Scores.created <= addtime(LAST_DAY(CURDATE()), '23:59:59')";
    }
    //remember to prefix any ambiguous columns (Users and Scores both have created, modified, and id columns)
    $query .= " ORDER BY score Desc, Arcade_Scores.created desc LIMIT 10"; //newest of the same score is ranked higher
...
{
    $query = "SELECT score from Arcade_Scores WHERE user_id = :id ORDER BY score desc LIMIT 1";
    $db = getDB();
    $stmt = $db->prepare($query);
    try {
...
function get_latest_scores($user_id, $limit = 10)
{
    ...
    $query = "SELECT score, created from Arcade_Scores where user_id = :id ORDER BY created desc LIMIT :limit";
    $db = getDB();
