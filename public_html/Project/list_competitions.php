...
$per_page = 5;
// paginate("SELECT count(1) as total FROM Arcade_Competitions WHERE expires > current_timestamp() AND did_payout < 1 AND did_calc < 1");
//handle page load

$stmt = $db->prepare("SELECT Arcade_Competitions.id, title, min_participants, current_participants, current_reward, expires, creator_id, min_score, join_cost, IF(competition_id is null, 0, 1) as joined,  CONCAT(first_place,'% - ', second_place, '% - ', third_place, '%') as place FROM Arcade_Competitions
JOIN Arcade_Payout_Options on Arcade_Payout_Options.id = Arcade_Competitions.payout_option
LEFT JOIN (SELECT * FROM Arcade_UserComps WHERE user_id = :uid) as uc ON uc.competition_id = Arcade_Competitions.id WHERE expires > current_timestamp() AND did_payout < 1 AND did_calc < 1 ORDER BY expires asc");
$results = [];
try {
